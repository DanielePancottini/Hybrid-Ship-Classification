...
class YOLOLoss(nn.Module):
    ...
    def get_losses(self, x_shifts, y_shifts, expanded_strides, labels, outputs):
        ...
        for batch_idx in range(outputs.shape[0]):
            # Filter the labels tensor to get only boxes for this batch_idx
            labels_per_image = labels[labels[:, 0] == batch_idx]
            
            num_gt = labels_per_image.shape[0] # Get the number of ground truths for this image
            if num_gt == 0:
                ...
                # labels_per_image format: [batch_idx, class_id, cx, cy, w, h]
                gt_bboxes_per_image = labels_per_image[:, 2:6].type_as(outputs) # [num_gt, 4] (cx, cy, w, h)
                gt_classes = labels_per_image[:, 1].type_as(outputs)            # [num_gt] (class_id)
    ...
    def get_assignments(self, num_gt, total_num_anchors, gt_bboxes_per_image, gt_classes, bboxes_preds_per_image,
        ...
        # -------------------------------------------------------#
        if self.fp16:
            with torch.autocast(enabled=False, device_type=cls_preds_.device.type):
